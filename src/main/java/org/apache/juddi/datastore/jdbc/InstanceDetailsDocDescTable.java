 ...
 */
class InstanceDetailsDocDescTable
{
  ...
  static
  {
   tablePrefix = Config.getStringProperty(
    ...
    sql.append("INSERT INTO ").append(tablePrefix).append("INSTANCE_DETAILS_DOC_DESCR (");
    ...
    sql.append("DESCR, ");
    sql.append("INSTANCE_DETAILS_DOC_DESCR_ID ");
    sql.append("FROM ").append(tablePrefix).append("INSTANCE_DETAILS_DOC_DESCR ");
    ...
    sql.append("DELETE FROM ").append(tablePrefix).append("INSTANCE_DETAILS_DOC_DESCR ");
    sql.append("WHERE BINDING_KEY=?");
    deleteSQL = sql.toString();
  }

  /**
   * Insert new row into the INSTANCE_DETAILS_DOC_DESCR table.
   *
   * @param bindingKey BindingKey to the BindingTemplate object
   * @param tModelInstanceInfoID int
    ...
    try
    {
      ...
      for (int descID = 0; descID < listSize; descID++)
      {
        ...
        if (log.isDebugEnabled()) {
            log.debug(
              "insert into " + tablePrefix + "INSTANCE_DETAILS_DOC_DESCR table:\n\n\t"
                + insertSQL
                + "\n\t BINDING_KEY="
                + bindingKey.toString()
    ...
    throws java.sql.SQLException
  {
    ...
    try
    {
      ...
      if (log.isDebugEnabled()) {
          log.debug(
            "select from " + tablePrefix + "INSTANCE_DETAILS_DOC_DESCR table:\n\n\t"
              + selectSQL
              + "\n\t BINDING_KEY="
              + bindingKey.toString()
              + "\n\t TMODEL_INSTANCE_INFO_ID="
  ...
  /**
   * Delete multiple rows from the INSTANCE_DETAILS_DOC_DESCR table
   * that are assigned to the BindingKey specified.
   *
   * @param bindingKey BindingKey
   * @param connection JDBC connection
   * @throws java.sql.SQLException
    ...
    throws java.sql.SQLException
  {
    ...
    try
    {
      ...
      if (log.isDebugEnabled()) {
          log.debug(
            "delete from " + tablePrefix + "INSTANCE_DETAILS_DOC_DESCR table:\n\n\t"
              + deleteSQL
              + "\n\t BINDING_KEY="
              + bindingKey.toString()
              + "\n");
