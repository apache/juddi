 ...
 */
public class InstanceDetailsHandler extends AbstractHandler
{
  public static final String TAG_NAME = "instanceDetails";

  private HandlerMaker maker = null;

  protected InstanceDetailsHandler(HandlerMaker maker)
  {
    this.maker = maker;
  }
  ...
  public RegistryObject unmarshal(Element element)
  {
    ...
    nodeList = XMLUtils.getChildElementsByTagName(element,InstanceParmsHandler.TAG_NAME);
    if (nodeList.size() > 0)
    {
      handler = maker.lookup(InstanceParmsHandler.TAG_NAME);
      obj.setInstanceParms((InstanceParms)handler.unmarshal((Element)nodeList.elementAt(0)));
    }
  ...
  if ((descrVector!=null) && (descrVector.size() > 0))
  {
    ...
    OverviewDoc overDoc = instDet.getOverviewDoc();
    if (overDoc != null)
    {
      handler = maker.lookup(OverviewDocHandler.TAG_NAME);
  ...
  InstanceParms instanceParms = instDet.getInstanceParms();
  if (instanceParms != null)
  {
    handler = maker.lookup(InstanceParmsHandler.TAG_NAME);
    handler.marshal(instanceParms,element);
  }
