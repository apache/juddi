 ...
 * in a J2EE Managed Environment. This is based on the JDBCDataStore
 * but without the reliance on commons-pooling and commons-dbcp.
 *
 *  @author Anil Saldhana (anil@apache.org)
 */
public class J2EEJDBCDataStore implements DataStore
{
  // private reference to the jUDDI logger
  private static Log log = LogFactory.getLog(J2EEJDBCDataStore.class);
  ...
  /**
   * Create a new JDBCDataStore and acquire a JDBC
   * connection from the connection pool.
   */
  public J2EEJDBCDataStore()
  {
    try {
       Context ctx = new InitialContext();
       String dsUrl = Config.getStringProperty("juddi.dataSource");
       if(dsUrl == null)
          throw new IllegalStateException("Configure juddi.datasource");

       ds = (DataSource)ctx.lookup(dsUrl);
       if( ds == null )
          throw new IllegalStateException("Datasource obtained is Null!");
       connection = ds.getConnection();
       if( connection == null)
          throw new IllegalStateException("Connection obtained is Null!");
    }
    catch(SQLException sqlex) {
      log.error("Exception occured while attempting to " +
        "acquire a JDBC connection: "+sqlex.getMessage());
    }
    catch (NamingException e)
    {
       log.error("Exception occured while attempting to " +
        "acquire a JDBC Datasource: "+e.getMessage());
    }
  }
